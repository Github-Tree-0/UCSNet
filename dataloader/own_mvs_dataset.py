...
class MVSTestSet(Dataset):
    ...
    def generate_pairs(self, ):
        ...
        for scene_name in self.scene_names:
            ...
            for i in range(cnt):
                ...
                candidates = pair_list[i * 2 + 2].split()
                nei_id = [int(candidates[2 * j + 1]) for j in range(self.num_views)]
                frame_idxes.append([ref_id, ] + nei_id)
            data_pairs.append({'scene_name': scene_name,
    ...
    def __getitem__(self, idx):
        ...
        scene_name = pair_dict['scene_name']
        frame_idxes = pair_dict['frame_idxes']
        ...
        frame_num = len(os.listdir(os.path.join(self.root_dir, scene_name, 'images')))
        for idx in range(frame_num):
            ...
            dp_values.append(np.array([min_dep, max_dep], np.float32))
